...
if [[ ${ENABLE_MSAN} -eq 1 ]]; then
    ...
    if ! cmake --build ${PATH_DEBUG_MEM} --config Debug "${BUILD_TARGETS_SANI[@]}"; then
        echo "${PATH_DEBUG_MEM} build failed"
        exit 1
    fi
else
    echo "MemorySanitizer is unsupported on $(uname -m); skipping"
fi
...
if [[ ${ENABLE_MSAN} -eq 1 ]]; then
    ...
    if ! cmake --build ${PATH_RELEASE_MEM} --config RelWithDebInfo "${BUILD_TARGETS_SANI[@]}"; then
...
export ASAN_OPTIONS=halt_on_error=1:detect_stack_use_after_return=1:check_initialization_order=1:strict_init_order=1
...
queue_sanitized_binary() {
    ...
    if [[ ${ENABLE_MSAN} -eq 1 ]]; then
        SANITIZER_TASKS+=(
            --task "${task_id}-debug-mem" "${label} - Debug Memory" "${PATH_DEBUG_MEM}/${binary}" -s
            --task "${task_id}-release-mem" "${label} - RelWithDebInfo Memory" "${PATH_RELEASE_MEM}/${binary}" -s
        )
    fi
}
