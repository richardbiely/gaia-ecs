...
/*!
Archetype chunk allocator. It's used when an archetype tries to allocate a new
chunk. Every time we're out of space this allocator creates a page of 16 MiB.
Each page consists of blocks of 1 MiB in size which are added or removed as
necessary. Each block then stores archetype chunks which are 16 kiB big
(CHUNK_SIZE) and therefore can take 64 chunks. This way we keep everything tight
in memory and avoid both unnecessary allocation and fragmentation.
*/
class ChunkAllocator {
  ...
  struct BlockHead {
    ...
    //! Number of used items. If equal to m_iMaxItems it is necessary to create
    //! a new block
  ...
  */
  void *Alloc() {
    ...
    if (pfree) {
      ...
      assert(((uintptr_t)freeHead + MemoryBlock::Size) >=
    ...
    utils::fill_array((uint32_t *)pfree,
                      (uint32_t)((CHUNK_SIZE + 3) / sizeof(uint32_t)),
                      0x7fcdf00dU);
#endif
  ...
  */
  size_t Free(void *pointer) {
    ...
    [[maybe_unused]] const auto offset =
        (uintptr_t)pointer - (uintptr_t)ChunkAllocator::BlockHeadData(*head);
    assert((offset % CHUNK_SIZE) == 0 &&
           "Attempt to deallocate mismatched pointer");
    assert(((uintptr_t)head + MemoryBlock::Size) >=
           (uintptr_t)pointer + CHUNK_SIZE);
#endif

#ifdef _DEBUG
    // Fill freed memory by 0xfeeefeee as MSVC does
    utils::fill_array((uint32_t *)pointer, (int)(CHUNK_SIZE / sizeof(uint32_t)),
                      0xfeeefeeeU);
#endif
...
private:
  ...
  void FreeBlock(BlockHead *head) {
    ...
    assert((uintptr_t)head >= (uintptr_t)page->data &&
