...
#define LListInsert(llist, item, link) \
	...
	if(llist.first) { llist.first->link.prev = &((item)->link.next); llist.first = item; } \
	(item)->link.prev = &llist.first; \
	llist.first = item; \
	llist.count++;

#define LListRemove(llist, item, link) *((item)->link.prev) = (item)->link.next; \
	if ((item)->link.next) \
	...
	llist.count--;
		...
		/*!
		Archetype chunk allocator. It's used when an archetype tries to allocate a new chunk.
		Every time we're out of space this allocator creates a page of 16 MiB.
		Each page consists of blocks of 1 MiB in size which are added or removed as necessary.
		Each block then stores archetype chunks which are 16 kiB big (CHUNK_SIZE)
		and therefore can take 64 chunks.
		This way we keep everything tight in memory and avoid both unnecessary allocation and fragmentation.
		*/
		class ChunkAllocator
		{
			struct MemoryPage;
			struct MemItem { MemItem* next; };
			...
			struct BlockHead
			{
				...
				//! Number of used items. If equal to m_iMaxItems it is necessary to create a new block
				uint16_t m_iUsedItems = 0;
			};

			static constexpr size_t BlockHeadSize = ((sizeof(BlockHead) + 15) & ~15);
			static void* BlockHeadData(BlockHead& head) { return (uint8_t*)&head + BlockHeadSize; }
			...
			struct MemoryBlock
			{
				...
				//! Number of allocations in block. Total memory = CHUNK_SIZE * m_iNumAllocations
			...
			struct MemoryPage
			{
				static constexpr uint32_t Size = 16 * MemoryBlock::Size; // 16 MiB per page
		...
		public:
			ChunkAllocator()
			{
				...
				m_Block.m_iMaxItems = (uint16_t)((MemoryBlock::Size - BlockHeadSize) / CHUNK_SIZE);
			}
			...
			*/
			void* Alloc()
			{
				...
				if (pfree)
				{
					...
					assert(((uintptr_t)freeHead + MemoryBlock::Size) >= (uintptr_t)pfree + CHUNK_SIZE);
				}
				else
				{
					// Initial allocation
...
#if GAIA_DEBUG
				[[maybe_unused]] const auto offset = (uintptr_t)pointer - (uintptr_t)ChunkAllocator::BlockHeadData(*head);
				assert((offset % CHUNK_SIZE) == 0 && "Attempt to deallocate mismatched pointer");
				assert(((uintptr_t)head + MemoryBlock::Size) >= (uintptr_t)pointer + CHUNK_SIZE);
#endif
			...
			*/
			void GetStats(SmallBlockAllocatorStats& stats) const
			{
				...
				stats.Overhead = (m_Block.m_iNumBlocks * MemoryBlock::Size) - stats.Allocated;
			}
			...
			*/
			void Flush()
			{
				...
				for (BlockHead* next, *head = m_Block.m_FreeBlocks.first; head; head = next)
				{
				...
				{
					next = page->m_Link.next;
					if (page->m_Blocks.count() == (MemoryPage::Size / MemoryBlock::Size))
					{
		...
		private:
			MemoryPage* AllocPage()
			{
				...
				uint8_t* firstBlock = (uint8_t*)BlockFromPtr(pagePtr + MemoryBlock::Size - 1);
				const uint32_t numBlocks = MemoryPage::Size / MemoryBlock::Size;
				for (int n = numBlocks - 1; n >= 0; n--)
				{
					BlockHead* blockHead = (BlockHead*)(firstBlock + MemoryBlock::Size * n);
			...
			void FreeBlock(BlockHead* head)
			{
				...
				assert((uintptr_t)head >= (uintptr_t)page->data && (uintptr_t)head < ((uintptr_t)page->data + MemoryPage::Size));
				...
				else if (page->m_Blocks.count() == (MemoryPage::Size / MemoryBlock::Size))
				{
