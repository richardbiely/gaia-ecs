...
private:
  ...
  // queries because as new components are added and removed the global type map
  ...
  EntityQuery &Commit(uint32_t worldVersion) {
    ...
    auto commit = [&](ComponentType componentType) {
      ...
      // the order in which components are provided Bubble sort is okay. We're
      // dealing with at most MAX_COMPONENTS_PER_ARCHETYPE items anyway. 99% of
      // time with at most 3 or 4
      // TODO: Replace with a sorting network
      std::sort(listNone.begin(), listNone.end(),
                std::less<const ComponentMetaData *>());
      std::sort(listAny.begin(), listAny.end(),
                std::less<const ComponentMetaData *>());
...
#if GAIA_DEBUG
          LOG_E("Trying to add ECS component %s to an already full ECS query!",
                metaType->name.data());
...
#if GAIA_DEBUG
        LOG_E("SetChangeFilter trying to filter ECS component %s but it's not "
              "a part of the query!",
              metaType->name);
#endif
        return false;
...
public:
  ...
  template <typename... TComponent> EntityQuery &WithNone() {
    CalculateHashes<TComponent...>(list[ComponentType::CT_Generic].listNone);
    return *this;
  }
  ...
  template <typename... TComponent> EntityQuery &WithChangedChunkComponents() {
    SetChangedFilter<TComponent...>(listChangeFiltered[ComponentType::CT_Chunk],
