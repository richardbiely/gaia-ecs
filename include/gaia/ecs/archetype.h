...
private:
  ...
  //! The number of entities this archetype can take (e.g 5 = 5 entities with
  ...
  Create(World &pWorld, tcb::span<const ComponentMetaData *> genericTypes,
    ...
    const auto maxGenericItemsInArchetype =
        (Chunk::DATA_SIZE - chunkComponentListSize) / genericComponentListSize;
...
public:
  ...
  template <typename... T> [[nodiscard]] bool HasAllChunkComponents() const {
  ...
  template <typename... T> [[nodiscard]] bool HasNoneChunkComponents() const {
