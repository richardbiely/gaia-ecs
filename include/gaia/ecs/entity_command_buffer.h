...
World::ForEachChunk can result in changes of archetypes or chunk structure. This
would lead to undefined behavior. Therefore, such operations have to be executed
...
class EntityCommandBuffer final {
  ...
  template <typename... TComponent> void AddComponent_Internal(Entity entity) {
    // Entity
    {
  ...
  void SetComponent_Internal(Entity entity, ECS_SMART_ARG(TComponent)... data) {
    // Verify components
    const ComponentMetaData *typesToAdd[] = {
    ...
    for (const auto type : typesToAdd) {
      if (type == nullptr)
        continue;
      ++validComponents;
      validSize += type->size + sizeof(void *); // meta data pointer + data size
    }
    ...
    // Components
    {
      ...
      // Component data
      (SetComponentData_Internal<TComponent>(lastIndex,
                                             std::forward<TComponent>(data)),
       ...);
    }
  }
  ...
  void RemoveComponent_Internal(Entity entity) {
    ...
    // Components
    {
      ...
      const auto lastIndex = m_data.size();
      m_data.resize(m_data.size() +
                    sizeof(ComponentMetaData *) * validComponentCount);
...
public:
  ...
  \return Entity that will be created. The id is not usable right away. It will
  be filled with proper data after Commit()
  */
  ...
  */
  template <typename... TComponent> void AddComponent(Entity entity) {
    ...
    if (!VerifyMaxComponentCountPerArchetype((uint32_t)sizeof...(TComponent)))
      return;
  ...
  */
  ...
  void SetComponentData(Entity entity, ECS_SMART_ARG(TComponent)... data) {
    ...
    m_data.push_back(ComponentType::CT_Generic);
    SetComponent_Internal<TComponent...>(entity,
                                         std::forward<TComponent>(data)...);
  }
    ...
    for (auto i = 0; i < m_data.size();) {
      ...
      case ADD_COMPONENT: {
        ...
        world->AddComponent_Internal(
            type, *entity, tcb::span(newMetatypes, (uintptr_t)componentCount));
      } break;
      case SET_COMPONENT: {
        // Type
        ...
        for (uint8_t j = 0; j < componentCount; ++j) {
          ...
          i += sizeof(const ComponentMetaData *);
          memcpy(chunk->GetComponent_Internal(componentType, index, type),
      ...
      case REMOVE_COMPONENT: {
        ...
        world->RemoveComponent_Internal(
            type, e, tcb::span(newMetatypes, (uintptr_t)componentCount));
      } break;
      }
    }
