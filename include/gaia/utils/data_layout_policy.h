...
template <class S, class Tupple> S TupleToStruct(Tupple &&tup) {
  ...
  return TupleToStruct<S>(std::make_index_sequence<std::tuple_size<T>{}>{},
...
template <class, class...> std::false_type CheckIsBracesConstructible(...);
template <class T, class... TArgs>
using IsBracesConstructible =
    decltype(CheckIsBracesConstructible<T, TArgs...>(0));

// Converts a struct to a tuple (struct necessary to support initialization via
// Struct{x,y,...,z})
template <class T> auto StructToTuple(T &&object) noexcept {
  using type = std::decay_t<T>;
  if constexpr (IsBracesConstructible<type, AnyType, AnyType, AnyType, AnyType,
                                      AnyType>{}) {
  ...
  // We also want compilation to fail for structs with many members so we can
  // handle them here That shouldn't be necessary, though for we plan tu support
  // only structs with little amount of arguments.
  // else
...
constexpr static unsigned CalculateSoAByteOffset(const uintptr_t address,
                                                 const unsigned size) {
  if constexpr (N == 0) {
    ...
    return sizeof(typename std::tuple_element<N - 1, Tuple>::type) * size +
...
struct DataViewPolicy<DataLayout::AoS, ValueType> {
  ...
  constexpr static void Set(tcb::span<ValueType> s, unsigned idx,
                            ValueType &&val) {
    SetInternal((ValueType *)s.data(), idx, std::forward<ValueType>(val));
  }

private:
  constexpr static ValueType GetInternal(const ValueType *data,
                                         const unsigned idx) {
    return data[idx];
  }
  constexpr static void SetInternal(ValueType *data, const unsigned idx,
                                    ValueType &&val) {
...
struct DataViewPolicy<DataLayout::SoA, ValueType> {
  constexpr static ValueType Get(tcb::span<ValueType> s, const unsigned idx) {
    auto t = StructToTuple(ValueType{});
    return GetInternal(
        t, s, idx,
        std::make_integer_sequence<unsigned,
                                   std::tuple_size<decltype(t)>::value>());
  }

  constexpr static void Set(tcb::span<ValueType> s, const unsigned idx,
                            ValueType &&val) {
...
private:
  ...
  GetInternal(Tuple &t, tcb::span<ValueType> s, const unsigned idx,
    ...
    (GetInternal<Tuple, Ids, typename std::tuple_element<Ids, Tuple>::type>(
         ...
         idx * sizeof(typename std::tuple_element<Ids, Tuple>::type) +
             CalculateSoAByteOffset<Ids, Tuple>((uintptr_t)s.data(), s.size())),
     ...);
    return TupleToStruct<ValueType, Tuple>(std::forward<Tuple>(t));
  }

  template <typename Tuple, unsigned Ids, typename TMemberType>
  constexpr static void GetInternal(Tuple &t, const char *data,
                                    const unsigned idx) {
  ...
  SetInternal(Tuple &t, tcb::span<TValue> s, const unsigned idx,
  ...
  constexpr static void SetInternal(char *data, const unsigned idx,
                                    TMemberValue val) {
