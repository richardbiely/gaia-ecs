...
/*! @brief Provides statically generated unique identifier. */
...
/*! @brief Provides statically generated unique identifier for a given group of types. */
template <typename...> class type_group {
  inline static uint32_t identifier{};

public:
  template <typename... Type> inline static const uint32_t id = identifier++;
};
...
struct type_info final {
  ...
  template <typename T> [[nodiscard]] static constexpr auto name() noexcept {
    // MSVC  : const char* __cdecl ecs::ComponentMetaData::GetMetaName<struct
    // ecs::EnfEntity>(void)
    //   res : ecs::EnfEntity
    // Clang : const ecs::ComponentMetaData::GetMetaName() [T = ecs::EnfEntity]
    //   res : ecs::EnfEntity
  ...
  template <typename T> [[nodiscard]] static constexpr auto hash() noexcept {
