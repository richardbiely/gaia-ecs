...
/**
 ...
 deleting them and guarantee the lifetime of the instance is longer than that of
 the delegate. \tparam Ret Return type of a function type. \tparam Args Types of
...
public:
  ...
  /**
   \brief Constructs a delegate by binding a free function or an unbound member
   to it. \tparam FuncToBind Function or member to bind to the delegate.
   */
  ...
  delegate(internal::connect_arg_t<FuncToBind>,
           Type &&value_or_instance) noexcept {
    bind<FuncToBind>(std::forward<Type>(value_or_instance));
  }

  /**
   \brief Constructs a delegate by binding a function with optional context to
   it. \param func Function to bind to the delegate. \param data User defined
   arbitrary data.
   */
  ...
  /**
   \brief Binds a free function with context or a bound member to a delegate.
  ...
  void bind(Type &value_or_instance) noexcept {
    ...
    if constexpr (std::is_invocable_r_v<Ret, decltype(FuncToBind), Type &,
                                        Args...>) {
      fnc = [](const void *context, Args... args) -> Ret {
        auto pType = static_cast<Type *>(
            const_cast<std::conditional_t<std::is_const_v<Type>, const void *,
                                          void *>>(context));
        return Ret(
  ...
  /**
   \brief Binds an user defined function with optional context to a delegate.
   The context is returned as the first argument to the target function in all
   cases. \param function Function to bind to the delegate. \param context User
   defined arbitrary ctx.
   */
  ...
  /**
   \brief The delegate invokes the underlying function and returns the result.
  ...
  [[nodiscard]] explicit operator bool() const noexcept {
    return fnc !=
           nullptr; // there's no way to set just ctx so it's enough to test fnc
  }
...
private:
  ...
  [[nodiscard]] auto wrap(Type &, std::index_sequence<Index...>) noexcept {
    return [](const void *context, Args... args) -> Ret {
      ...
      auto pType = static_cast<Type *>(
          const_cast<
              std::conditional_t<std::is_const_v<Type>, const void *, void *>>(
              context));
      return Ret(std::invoke(
          FuncToBind, *pType,
          std::forward<std::tuple_element_t<Index, std::tuple<Args...>>>(
...
template <typename Ret, typename... Args>
delegate(Ret (*)(const void *, Args...),
         const void * = nullptr) noexcept->delegate<Ret(Args...)>;
...
/**
 ...
 users of the class. It also allows for less header files contention because any
 binding can (and should) happen in source files. \warning Lifetime of any sink
 must not be longer than that of the signal to which it refers. \tparam Ret
 Return type of a function type. \tparam Args Types of arguments of a function
 type.
 */
...
public:
  ...
  /**
   \brief Moves signals from another sink to this one. Result is stored in this
   object. The sink we merge from is cleared. \param other Sink to move signals
   from
   */
  ...
  /**
   ...
   The signal handler performs checks to avoid multiple connections for the same
   function. \tparam FuncToBind Function or member to bind to the signal.
   */
   ...
   */
  template <auto FuncToBind, typename Type> void bind(Type &value_or_instance) {
  ...
  /**
   ...
   The context is returned as the first argument to the target function in all
   cases. \param function Function to bind to the signal. \param context User
   defined arbitrary ctx.
   */
  ...
  /**
   \brief Unbinds a free function with context or a bound member from a signal.
   \tparam FuncToUnbind Function or member to unbind from the signal.
   \tparam Type Type of class or type of context.
   \param value_or_instance A valid object that fits the purpose.
   */
  ...
  /**
   \brief Unbinds a free function with context or bound members from a signal.
   \tparam Type Type of class or type of context.
   \param value_or_instance A valid object that fits the purpose.
   */
